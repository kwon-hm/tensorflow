...
epochs = 10
THRESHOLD = 0.05
path = 'AD 실제train&예측train 비교/'
name = 'checkpoint(e-100, b-64, t-10)'
...
num = 0
while num < TIME_STEPS*50:
    db_data = df_train[num : num+1]
    df_Cpu = df_train[num : num+1]['Cpu']
    df_Mem = df_train[num : num+1]['Mem']
    df_Network = df_train[num : num+1]['Network']
    df_Date = df_train.index[num : num+1]
    ...
    if(len(time_) == TIME_STEPS):
        ...
        df_data = scaler.inverse_transform(df_train[num+1 : num+2]) # 실제
        predic = scaler.inverse_transform(tp[0]) # 예측
        print(num, " 실제 : ", df_data[0][0], df_data[0][1], df_data[0][2], df_train.index[num+1])
        print(num, " 예측 : ", predic[-1][0], predic[-1][1], predic[-1][2])
        print()
        test_predic_t.append(df_train.index[num+1])
        test_predic_c.append(predic[0][0])
        test_predic_m.append(predic[0][1])
        test_predic_n.append(predic[0][2])

        df_old_t.append(df_train.index[num+1])
