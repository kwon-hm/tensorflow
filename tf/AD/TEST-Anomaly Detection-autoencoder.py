...
epochs = 100
THRESHOLD = 0.097
checkpoint_name="AD/checkpoint(e-100, b-64, t-60)"
...
checkpoint_path = "AD/checkpoint/cp.ckpt"
...
# print('2: ', np.mean(np.square(X_train_pred - x_train), axis=2)[0])
...
while training_data_len < len(df) - TIME_STEPS:
    db_data = conn.find({'dynamicStat': 1}).skip(training_data_len).limit(1)
