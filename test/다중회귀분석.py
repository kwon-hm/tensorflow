...
#입력:xs-numpy array (n차)x값들 벡터 , ws-numpy array (n차)w값들 벡터, b 스칼라
#반환: 스칼라
def 예상하다(xs, ws, b):
    return np.add(np.dot(xs, ws), b)
#입력:xs-numpy array (n차)x값들 벡터 , ws-numpy array (n차)w값들 벡터, b 스칼라, 실제값: 스칼라 
#반환: 스칼라
def 에러구하다(xs, ws, b, 실제값):
    return np.subtract(실제값 , 예상하다(xs, ws, b))
#입력:xs-numpy array (n차)x값들 벡터 , ws-numpy array (n차)w값들 벡터, b 스칼라, 실제값: 스칼라 
#반환: 스칼라
def 에러비용구하다(xs, ws, b, 실제값): #자승의 합에 의한, 에러율 제곱의 
        return np.sum(np.square(에러구하다(xs, ws, b, 실제값)))
#입력:xs-numpy array (n차)x값들 벡터 , ws-numpy array (n차)w값들 벡터, b 스칼라, 실제값: 스칼라    
#반환: (w변화에대한에러변화율들:input_size차 벡터 , b변화에대한에러변화율_스칼라)인 튜플    
def w들과b에대한에러변화율을구하다(xs, ws, b, 실제값):# 수치미분이용
    h = 1e-4
    w변화에대한에러변화율들= np.zeros_like(ws)
    for i in range(len(xs)):
        원래w_i값 = ws[i]
        ws[i]= 원래w_i값 + h
        에러비용2 = 에러비용구하다(xs, ws, b, 실제값)
        ws[i]= 원래w_i값 - h
        에러비용1 = 에러비용구하다(xs, ws, b, 실제값)        
        w변화에대한에러변화율들[i] = (에러비용2 - 에러비용1) / (2 * h) 
        ws[i] = 원래w_i값
    b변화에대한에러변화율 = (에러비용구하다(xs, ws, b+h , 실제값) - 에러비용구하다(xs, ws, b-h, 실제값)) / (2 * h) 
...
for xs_i, 실제값 in zip(xs,ts):
    ws, b = 최소오류율위한w들과b를구하다(xs_i, ws, b, 실제값)
