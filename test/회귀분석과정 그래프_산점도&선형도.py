# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 14:19:51 2018
...
xs = np.array([ x for x in range(1, 101) if x % 2 == 0])
ts = np.array([ f(x)+np.random.uniform(-30,30) for x in xs ])
...
def 에러비용구하다(x_batch, w, b, 실제값): #자승의 합에 의한, 에러율 제곱의 
        return 0.05 * np.sum(np.square(에러구하다(x_batch, w, b, 실제값)))
...
for i in range(len(ts)//batch_size):
    ...
    plt.scatter(xs, ts)
    ys = 예상하다(xs, w, b)
    plt.plot(xs, ys, color='red', linestyle='solid')
    plt.axis("equal")
    plt.show()
    
#Test

for i, (x, 실제값) in enumerate(zip(xs,ts)):
    if i % 5 == 0:
        y = 예상하다(x, w, b)
        print('예상:',y, '실제:', 실제값)    
